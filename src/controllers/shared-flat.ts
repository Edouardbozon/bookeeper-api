 ...
 * GET /shared-flat
 */
export const getSharedFlat = asyncMiddleware(
 ...
 * GET /shared-flat/{id}
 */
export const getSharedFlatDetail = asyncMiddleware(
  async (req: Request, res: Response, next: NextFunction) => {
    if (!req.params.id) {
 ...
 * POST /shared-flat
 */
export const createSharedFlat = asyncMiddleware(
  async (req: Request, res: Response, next: NextFunction) => {
    req.assert("name", "Name is not valid").notEmpty();
    req.assert("size", "Size must be an integer").isInt();
    req
      .assert("pricePerMonth", "PricePerMonth must be an integer")
      .optional()
      .isInt();
 ...
 */
export const deleteSharedFlat = asyncMiddleware(
  async (req: Request, res: Response, next: NextFunction) => {
    if (!req.params.id) {
      return res.status(400).json(format("Missing {id} param"));
    }
 ...
 * PUT /shared-flat/{id}
 */
export const putSharedFlat = asyncMiddleware(
  async (req: Request, res: Response, next: NextFunction) => {
    if (!req.params.id) {
      return res.status(400).json(format("Missing {id} param"));
    }
    ...
    req
      .assert("iconUrl", "iconUrl is not valid")
      .notEmpty()
      .isURL();
    req
      .assert("bannerUrl", "bannerUrl is not valid")
      .notEmpty()
      .isURL();

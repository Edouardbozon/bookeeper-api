...
import {
  ...
  JoinRequestModel,
} from "../models/Shared-flat/Join-request";
import { default as User, UserModel } from "../models/User/User";

/**
 * GET /shared-flat/{id}/join
 */
export const getJoinSharedFlatRequest = asyncMiddleware(
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    // @todo filter out accepted or rejected join requests
 ...
 */
export const postJoinSharedFlatRequest = asyncMiddleware(
  async (req: Request, res: Response) => {
    if (!req.params.id) {
      return res.status(400).json(format("Missing {id} param"));
    }

    const hasAlreadyRequested = (await JoinRequest.findOne({
      userId: req.user.id,
    })) as JoinRequestModel;
 ...
 */
export const postValidateJoinRequest = asyncMiddleware(
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    await user.acceptOrReject(
      req.params.joinRequestId,
      req.params.sharedFlatId,
      "accepted",
    );
 ...
 */
export const postRejectJoinRequest = asyncMiddleware(
  async (req: Request, res: Response) => {
    if (!req.params.sharedFlatId) {
      return res.status(400).json(format("Missing {sharedFlatId} param"));
    }
